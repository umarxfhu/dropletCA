    ...
    def run(self):
        ...
        while(1):
            ...
            if k == 27:         # esc to exit
                ...
                self.output = cv.cvtColor(self.output, cv.COLOR_BGR2GRAY)
                self.output = cv.blur(self.output, (4, 4), 0)
                self.output = cv.adaptiveThreshold(self.output, 255, cv.ADAPTIVE_THRESH_MEAN_C, \
                            cv.THRESH_BINARY, 11, -2)


                edged = autoCanny(self.output)
                start_x=self.rect[0]
                start_y=self.rect[1]
                width=self.rect[2]
                height=self.rect[3]
                ...
                color = (70,255,0)
                # draw horizon and contact points
